...
PATH_TRAIN = './train/combined/'
PATH_TEST = './test/combined/'
...
class Pix2pix(tf.Module):
    ...
    def discriminator_loss(self, disc_target_output, disc_generated_output):
        target_loss = self.loss_object(tf.ones_like(disc_target_output), disc_target_output)
    ...
    def train(self, dataset, checkpoint_pr):
        ...
        for epoch in range(self.epochs):
            ...
            print(f'Generator Loss: {gen_loss:.3f} | Discriminator Loss: {disc_loss:.3f}')
...
def main():
    ...
    '''
    inp, tgt = load(PATH_TRAIN + str(1) + '.png')

    plt.figure()
    plt.imshow(inp/255.0)
    plt.title('train T1')
    plt.figure()
    plt.imshow(tgt/255.0)
    plt.title('train T2')
    plt.show()
    ...
    parser.add_argument('-f', dest='input', help='Path to input folder', required=True)
    parser.add_argument('-m', dest='mode', help='Mode to run MRI image converison', choices=['train', 'convert'], required=True)
    
    
    parser.add_argument('-c', dest='convert', help='Image translation direction',  choices=['t1_to_t2', 't2_to_t1'], required=True)
    ...
    parser.add_argument('-l_t1_t2', dest='load_t1_t2', help='Load saved model T1 to T2 from custom directory')
    parser.add_argument('-l_t2_t1', dest='load_t2_t1', help='Load saved model T2 to T1 from custom directory')
    ...
    if args.mode == 'train':
        ...
        if args.convert == 't1_to_t2':
            train_dataset = train_dataset.map(load_image_train_t1_to_t2, num_parallel_calls=tf.data.AUTOTUNE)
        elif args.convert == 't2_to_t1':
            train_dataset = train_dataset.map(load_image_train_t2_to_t1, num_parallel_calls=tf.data.AUTOTUNE)
        ...
        test_dataset = tf.data.Dataset.list_files(args.input + '*.png', shuffle=False)
        ...
        for input_image in test_dataset:
            save_generated_images(saved_model.generator, input_image, (args.output + str(i) + '.png'))
            i += 1
